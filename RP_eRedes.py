def run_pipeline():
    ...
    for fls in all_files:
        ...
        #Insert data and count the rows
        total_rows= 0
        total_rows += insertToDB(df)
    
    print(f"... {total_rows} rows inserted into {table_name} for {db}")
    clts.elapt[f"... {total_rows} rows inserted into {table_name} for {db}"] = clts.deltat(tstart)
